...
with open('background_support_data.json', 'r') as file:
    patient_dict = json.load(file)

# --- Agent Configuration ---

AGENT_SEED = "background_seed" # Keep seed the same to maintain address
AGENT_PORT = 8002
AGENT_NAME = "background_agent" # Renamed agent

# --- Models ---

class BackgroundData(Model): # Renamed model
    message: str
...
async def fetch_background_plan():
    """Calls the Groq API to generate a background-info-based plan."""
    if not groq_client:
...
# Create the Demographics Agent instance
...
@background_agent.on_event("startup")
...
@background_agent.on_message(model=BackgroundData)
async def handle_demographics_request(ctx: Context, sender: str, msg: BackgroundData):
    """Handles requests for the demographics plan."""
    ctx.logger.info(f"Received message from {sender}: {msg.message}")
    ctx.logger.info("Generating demographics recovery plan...")
    plan = await fetch_background_plan()
    ctx.logger.info("Sending plan back...")
    try:
        await ctx.send(sender, BackgroundData(message=plan))
    except Exception as e:
